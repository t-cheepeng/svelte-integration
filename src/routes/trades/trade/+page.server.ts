...
import { getMillicentsFromDollarAndCents } from '$lib/utils/utils';
...
export const actions = {
	trade: async (event) => {
		...
		const pricePerUnitInMilli = formData.get(FormFields.PRICE_PER_UNIT_IN_MILLI);
		...
		const convertedPricePerUnitMilli = getMillicentsFromDollarAndCents(
			(pricePerUnitInMilli as FormDataEntryValue).toString()
		);
		...
		const tradeStockRequest: TradeStockRequest = {
			...
			pricePerUnitInMilli: convertedPricePerUnitMilli,
		...
		if (feeInMilli !== null) {
			...
			tradeStockRequest.feeInMilli = getMillicentsFromDollarAndCents(feeString);
		}
